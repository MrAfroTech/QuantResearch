...
export async function sendInsufficientBudgetTelegram(ticker, budgetRemaining, requiredCost) {
  const left = Number(budgetRemaining).toFixed(2);
  const needed = Number(requiredCost).toFixed(2);
  await sendTelegram(
    `⚠️ Not enough budget for next trade — $${left} left, needed ~$${needed} for ${ticker}`
  );
}
