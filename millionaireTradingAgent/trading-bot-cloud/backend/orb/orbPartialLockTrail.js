 ...
 *
 * ORB/Premarket now flatten the full book at the first trigger (no scale-outs),
 ...
 *
 * Floor = peak_mfe / ORB_PARTIAL_LOCK_TRAIL_DIVISOR (ratchets up only as MFE makes new highs).
 ...
 */
export function evaluateOrbPartialLockTrail({
  pnlFrac,
  mfeFrac,
  exitPhase,
  activationMfe = ORB_PARTIAL_LOCK_ACTIVATION_MFE,
  hardStopPct = null,
  trailDivisor = ORB_PARTIAL_LOCK_TRAIL_DIVISOR,
}) {
  ...
  if (unrealized <= trailFloor) {
    return {
      action: 'close_all',
      reason: ORB_PARTIAL_LOCK_CLOSE_REASON,
      peakMfe: peak,
      trailFloor,
      pnlFrac: unrealized,
    };
 ...
 */
export function shouldRaiseOrbPartialLockBrokerStop(position, trailFloor) {
 ...
 */
export function resolveOrbPartialLockStopFillReason({
  position,
  defaultReason,
}) {
  ...
  if (milestones === 0 && Number.isFinite(stopFrac) && stopFrac > 0) {
    return ORB_PARTIAL_LOCK_CLOSE_REASON;
  }
